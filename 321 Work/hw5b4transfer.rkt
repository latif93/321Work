...
(thn nFWAE?)
(els nFWAE?)]
[nW-fun (params (listof symbol?))
(body nFWAE?)]
[nW-app (fun-expr nFWAE?)
(arg-exprs (listof nFWAE?))])

(define (nparse s-expr)
  (cond [(number? s-expr)
         (nW-num s-expr)]
        [(symbol? s-expr)
         (nW-id s-expr)]
        [(list? s-expr)
         (when (empty? s-expr)
           (error 'nparse "the empty list is not a valid FWAE"))
         (case (first s-expr)
           [(+)
            (check-pieces s-expr 3 "+")
            (nW-add (nparse (second s-expr))
                   (nparse (third s-expr)))]
           [(-)
            (check-pieces s-expr 3 "-")
            (nW-sub (nparse (second s-expr))
                   (nparse (third s-expr)))]
           [(with)
            (check-pieces s-expr 3 "with")
            (check-pieces (second s-expr) 2 "with binding pair")
            (unless (symbol? (first (second s-expr)))
              (error nparse "expected variable name, got ~a" (first (second s-expr))))
            (nW-with (first (second s-expr))
                    (nparse (second (second s-expr)))
                    (nparse (third s-expr)))]
           [(if0)
          ;  (check-pieces s-expr 3 "if0")
           (nW-if0
            (nparse(second s-expr))
            (nparse(third s-expr))
            (nparse(fourth s-expr)))
                ]
           [(fun)
            (check-pieces s-expr 3 "fun")
            (nW-fun (second s-expr)
                   (nparse (third s-expr)))]
           [else
        ;    (check-pieces s-expr 2 "app")
            (nW-app (nparse (first s-expr)) (map nparse (rest s-expr)))])]
        [else
         (error 'nparse "expected FWAE, got ~a" s-expr)]))

(define (ncompile an-fwae)
  (type-case nFWAE an-fwae
    [nW-num (n) (num n)]
    [nW-add (l r) (maybe-constant-fold
                  +
                  add
                  (ncompile l)
                  (ncompile r))]
    [nW-sub (l r) (maybe-constant-fold
                  -
                  sub
                  (ncompile l)
                  (ncompile r))]
    [nW-with (name named-expr body)
            (app (fun name (ncompile body))
                 (ncompile named-expr))]
    [nW-id (name) (id name)]
    [nW-fun (param-name body)
            (if(empty? param-name)
               (error 'ncompile "nullary function")
            (if (empty? (rest param-name))
                (fun (first param-name) (ncompile body))
           (fun (first param-name) (ncompile(nW-fun (rest param-name) body)))))]
    [nW-if0 (tst thn els)
            (if0 (ncompile tst) (ncompile thn) (ncompile els))]
    [nW-app (fun-expr arg-expr)
            (if (empty? arg-expr)
                (error 'ncompile "nullary application")
            (if (empty? (remove (last arg-expr) arg-expr))
           (app (ncompile fun-expr)
                (ncompile (first arg-expr)))
           (app  (ncompile (nW-app fun-expr (remove (last arg-expr) arg-expr)))(ncompile (last arg-expr))) ))]))
;; ----------------------------------------------------------------------
...
(define (interp-expr an-fnwae)
  (interp an-fnwae initial-def-sub))
...
(test (ncompile(nparse `{f x y z})) (app (app (app (id 'f) (id 'x)) (id 'y)) (id 'z)))
(test/exn(ncompile(nparse `{{{f } y}z}))"nullary application")
(test/exn(ncompile(nparse `{fun {} {-{+ a b}x}}))"nullary function")
(test(ncompile(nparse `{fun {a b x} {-{+ a b}x}}))
...
(test(interp-expr(ncompile(nparse `{,factorial 5})))(numV 120))
(test(interp-expr(ncompile(nparse `{,factorial 4})))(numV 24))
(test(interp-expr(ncompile(nparse `{,factorial 3})))(numV 6))
(test(interp-expr(ncompile(nparse `{,factorial 2})))(numV 2))
(test(interp-expr(ncompile(nparse `{,factorial 1})))(numV 1))
(test/exn (interp-expr (ncompile (nparse `{1 2})))
          "expected function")

(test/exn (interp-expr (ncompile (nparse `x)))
          "free identifier")

(test/exn (interp-expr (ncompile (nparse `{+ 1 {fun {x} x}})))
          "expected number")

(test/exn (interp-expr (ncompile (nparse `{+ {fun {x} x}
{1 2}})))
"expected function")
(define prime? `{fun {q}
  {with {mk-rec
...
;(interp-expr(ncompile(nparse `{,prime? 0})))
(test(interp-expr(ncompile(nparse `{,prime? 1}))) (numV 0))
(test(interp-expr(ncompile(nparse `{,prime? 2}))) (numV 0))
(test(interp-expr(ncompile(nparse `{,prime? 3}))) (numV 0))
(test(interp-expr(ncompile(nparse `{,prime? 4}))) (numV 1))
(test(interp-expr(ncompile(nparse `{,prime? 5}))) (numV 0))
(test(interp-expr(ncompile(nparse `{,prime? 6}))) (numV 1))
(test(interp-expr(ncompile(nparse `{,prime? 7}))) (numV 0))
(test(interp-expr(ncompile(nparse `{,prime? 8}))) (numV 1))
(test(interp-expr(ncompile(nparse `{,prime? 9}))) (numV 1))
(test(interp-expr(ncompile(nparse `{,prime? 10}))) (numV 1))
